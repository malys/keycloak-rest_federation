...
public class RestUserFederationProviderFactory implements UserStorageProviderFactory<RestUserFederationProvider>, ImportSynchronization {
    ...
    public RestUserFederationProvider create(KeycloakSession session, ComponentModel model) {
        ...
        final String passwordAlgorithm = EnvSubstitutor.envStrSubstitutor.replace(model.getConfig().getFirst(PASSWORD_HASH_ALGORITHM)).toLowerCase();
        final Integer passwordIteration = Integer.parseInt(EnvSubstitutor.envStrSubstitutor.replace(model.getConfig().getFirst(PASSWORD_HASH_ITERATION)));
